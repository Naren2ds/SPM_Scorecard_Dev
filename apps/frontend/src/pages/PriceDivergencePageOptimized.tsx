...
type PdivLevel = "parent" | "supplier" | "zone" | "category";
type FormulaMode = "softStretch" | "strict";

interface PdivConfig {
  maxScore: number;
...
interface PriceDivergencePageProps {
  sharedParent: string[];
...
const LEVELS: Array<{ id: PdivLevel; label: string; note: string }> = [
...
function validateConfig(config: PdivConfig) {
  const errors: string[] = [];
  ...
  if (config.criticalFloor <= config.target) errors.push("Critical Floor must be greater than Target for Price Divergence.");
  return errors;
}
...
function PriceDivergencePageOptimized({ sharedParent, onParentChange }: PriceDivergencePageProps) {
  ...
  const [level, setLevel] = useState<PdivLevel>("parent");
  const [page, setPage] = useState(1);
  const [results, setResults] = useState<PdivResultsResponse | null>(null);
  const [summary, setSummary] = useState<PdivSummary | null>(null);
  const [savedConfig, setSavedConfig] = useState<PdivConfig>(DEFAULT_CONFIG);
  const [draftConfig, setDraftConfig] = useState<PdivConfig>(DEFAULT_CONFIG);
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchJson<{ config: PdivConfig }>(`${API_BASE}/api/pdiv/config`, { signal: controller.signal }),
      fetchJson<PdivFilterOptions>(`${API_BASE}/api/pdiv/filters`, { signal: controller.signal }),
    ])
  ...
  useEffect(() => {
    ...
    setError("");
    Promise.all([
      fetchJson<PdivResultsResponse>(`${API_BASE}/api/pdiv/results?${params}`, { signal: controller.signal }),
      fetchJson<PdivSummary>(`${API_BASE}/api/pdiv/summary?${summaryParams}`, { signal: controller.signal }),
    ])
  ...
  useEffect(() => {
    ...
    params.set("limit", "30");
    if (activePreviewId) params.set("preview_id", activePreviewId);
    fetchJson<{ items: SearchResult[] }>(`${API_BASE}/api/pdiv/search?${params}`, { signal: controller.signal })
  ...
  const runPreview = async () => {
    ...
    setMessage("Building a temporary Price Divergence preview for this cohort...");
    setError("");
    try {
  ...
  const discardPreview = async () => {
    if (preview) await fetch(`${API_BASE}/api/pdiv/preview/${preview.id}`, { method: "DELETE" }).catch(() => undefined);
    setPreview(null);
  ...
  const applyPreview = async () => {
    ...
    setScenarioBusy(true);
    setMessage("Applying Price Divergence settings and rebuilding the official scorecard...");
    setError("");
    try {
      await fetchJson(`${API_BASE}/api/scorecard/config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ kpiId: "PDIV", floor: draftConfig.criticalFloor, target: draftConfig.target, maxScore: draftConfig.maxScore }),
      });
      ...
      setPreview(null);
      setPage(1);
      setMessage("Price Divergence Soft Stretch settings applied to the official scorecard.");
    } catch (requestError) {
  ...
  const exportResults = () => {
    ...
    if (activePreviewId) params.set("preview_id", activePreviewId);
    window.location.assign(`${API_BASE}/api/pdiv/export?${params}`);
  };
  ...
  return (
    <>
      <section className="top-bar kpi-page-heading">
        <div>
          <p className="eyebrow">Operational KPI</p>
          <h1>Price Divergence</h1>
          <p className="kpi-value-note">Lower is better. Parent Rollup loads first; Supplier, Zone, and Category levels calculate on demand.</p>
        </div>
      ...
      <section className="dot-summary-grid">
        ...
        <SummaryCard label="Average divergence" value={summary ? `${summary.average_divergence_pct.toFixed(2)}%` : "-"} />
      ...
      <section className="results-panel">
        <div className="dot-level-tabs" role="tablist" aria-label="Price Divergence result level">{LEVELS.map((item) => <button type="button" role="tab" aria-selected={level === item.id} className={level === item.id ? "is-active" : ""} key={item.id} onClick={() => { setLevel(item.id); setPage(1); setResultQuery(""); }}><strong>{item.label}</strong><span>{item.note}</span></button>)}</div>
        ...
        {error ? <div className="validation-box"><p>{error}</p></div> : loading ? <div className="empty-state">Loading the selected Price Divergence result level...</div> : !results?.items.length ? <div className="empty-state">No Price Divergence results match these filters.</div> : <PdivResultsTable rows={results.items} level={level} />}
